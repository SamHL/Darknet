...
layer make_batchnorm_layer(int batch, int w, int h, int c)
{
    ...
    l.w = l.out_w = w;
    l.c = l.out_c = c;
    l.output = static_cast<float*>(calloc(h * w * c * batch, sizeof(float)));
    l.delta  = static_cast<float*>(calloc(h * w * c * batch, sizeof(float)));
    l.inputs = w*h*c;
    l.outputs = l.inputs;

    l.scales = static_cast<float*>(calloc(c, sizeof(float)));
    l.scale_updates = static_cast<float*>(calloc(c, sizeof(float)));
    l.biases = static_cast<float*>(calloc(c, sizeof(float)));
    l.bias_updates = static_cast<float*>(calloc(c, sizeof(float)));
    int i;
    for(i = 0; i < c; ++i){
        l.scales[i] = 1;
    }

    l.mean = static_cast<float*>(calloc(c, sizeof(float)));
    l.variance = static_cast<float*>(calloc(c, sizeof(float)));

    l.rolling_mean = static_cast<float*>(calloc(c, sizeof(float)));
    l.rolling_variance = static_cast<float*>(calloc(c, sizeof(float)));
