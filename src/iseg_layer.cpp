...
layer make_iseg_layer(int batch, int w, int h, int classes, int ids)
{
    ...
    l.counts = static_cast<int*>(calloc(90, sizeof(int)));
    l.sums = static_cast<float**>(calloc(90, sizeof(float*)));
    if(ids){
        int i;
        for(i = 0; i < 90; ++i){
            l.sums[i] = static_cast<float*>(calloc(ids, sizeof(float)));
        }
    }
...
void forward_iseg_layer(const layer l, network net)
{
    ...
    for (b = 0; b < l.batch; ++b){
        ...
        float *mse = static_cast<float*>(calloc(90, sizeof(float)));
        for(i = 0; i < 90; ++i){
            int c = net.truth[b*l.truths + i*(l.w*l.h+1)];
            if(c < 0) break;
            for(k = 0; k < l.w*l.h; ++k){
