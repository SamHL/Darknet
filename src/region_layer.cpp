...
layer make_region_layer(int batch, int w, int h, int n, int classes, int coords)
{
    ...
    l.classes = classes;
    l.coords = coords;
    l.cost = static_cast<float*>(calloc(1, sizeof(float)));
    l.biases = static_cast<float*>(calloc(n*2, sizeof(float)));
    l.bias_updates = static_cast<float*>(calloc(n*2, sizeof(float)));
    l.outputs = h*w*n*(classes + coords + 1);
    l.inputs = l.outputs;
    l.truths = 30*(l.coords + 1);
    l.delta = static_cast<float*>(calloc(batch*l.outputs, sizeof(float)));
    l.output = static_cast<float*>(calloc(batch*l.outputs, sizeof(float)));
    int i;
    for(i = 0; i < n*2; ++i){
        l.biases[i] = .5;
    }
...
void delta_region_class(float *output, float *delta, int index, int classC, int classes, tree *hier, float scale, int stride, float *avg_cat, int tag)
{
    int i, n;
    if(hier){
        float pred = 1;
        while(classC >= 0){
            pred *= output[index + stride*classC];
            int g = hier->group[classC];
            int offset = hier->group_offset[g];
            for(i = 0; i < hier->group_size[g]; ++i){
                delta[index + stride*(offset + i)] = scale * (0 - output[index + stride*(offset + i)]);
            }
            delta[index + stride*classC] = scale * (1 - output[index + stride*classC]);

            classC = hier->parent[classC];
        }
        *avg_cat += pred;
    } else {
        if (delta[index] && tag){
        ...
        for(n = 0; n < classes; ++n){
            delta[index + stride*n] = scale * (((n == classC)?1 : 0) - output[index + stride*n]);
            if(n == classC) *avg_cat += output[index + stride*n];
        }
    }
}
    ...
    for (b = 0; b < l.batch; ++b) {
        if(l.softmax_tree){
            ...
            for(t = 0; t < 30; ++t){
                ...
                if(!truth.x) break;
                int classC = net.truth[t*(l.coords + 1) + b*l.truths + l.coords];
                float maxp = 0;
                int maxi = 0;
                if(truth.x > 100000 && truth.y > 100000){
                    for(n = 0; n < l.n*l.w*l.h; ++n){
                        ...
                        float p = scale*get_hierarchy_probability(l.output + class_index, l.softmax_tree, classC, l.w*l.h);
                        if(p > maxp){
                            maxp = p;
                            maxi = n;
                        }
                    ...
                    delta_region_class(l.output, l.delta, class_index, classC, l.classes, l.softmax_tree, l.class_scale, l.w*l.h, &avg_cat, !l.softmax);
