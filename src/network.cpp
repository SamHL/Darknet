...
{
    network *net = static_cast<network*>(calloc(1, sizeof(network)));
    net->n = n;
    net->layers = static_cast<layer*>(calloc(net->n, sizeof(layer)));
    net->seen = static_cast<size_t*>(calloc(1, sizeof(size_t)));
    net->t    = static_cast<int*>(calloc(1, sizeof(int)));
    net->cost = static_cast<float*>(calloc(1, sizeof(float)));
    return net;
}
...
int resize_network(network *net, int w, int h)
{
    ...
    free(net->input);
    free(net->truth);
    net->input = static_cast<float*>(calloc(net->inputs*net->batch, sizeof(float)));
    net->truth = static_cast<float*>(calloc(net->truths*net->batch, sizeof(float)));
#ifdef GPU
    if(gpu_index >= 0){
        ...
        free(net->workspace);
        net->workspace = static_cast<float*>(calloc(1, workspace_size));
    }
#else
    free(net->workspace);
...
detection *make_network_boxes(network *net, float thresh, int *num)
{
    ...
    if(num) *num = nboxes;
    detection *dets = static_cast<detection*>(calloc(nboxes, sizeof(detection)));
    for(i = 0; i < nboxes; ++i){
        dets[i].prob = static_cast<float*>(calloc(l.classes, sizeof(float)));
        if(l.coords > 4){
            dets[i].mask = static_cast<float*>(calloc(l.coords-4, sizeof(float)));
        }
    }
    return dets;
}
...
matrix network_predict_data_multi(network *net, data test, int n)
{
    ...
    float *X = static_cast<float*>(calloc(net->batch*test.X.rows, sizeof(float)));
...
matrix network_predict_data(network *net, data test)
{
    ...
    int k = net->outputs;
    matrix pred = make_matrix(test.X.rows, k);
    float *X = static_cast<float*>(calloc(net->batch*test.X.cols, sizeof(float)));
    for(i = 0; i < test.X.rows; i += net->batch){
        for(b = 0; b < net->batch; ++b){
            if(i+b == test.X.rows) break;
            memcpy(X+b*test.X.cols, test.X.vals[i+b], test.X.cols*sizeof(float));
