...
layer make_normalization_layer(int batch, int w, int h, int c, int size, float alpha, float beta, float kappa)
{
    ...
    layer.alpha = alpha;
    layer.beta = beta;
    layer.output = static_cast<float*>(calloc(h * w * c * batch, sizeof(float)));
    layer.delta = static_cast<float*>(calloc(h * w * c * batch, sizeof(float)));
    layer.squared = static_cast<float*>(calloc(h * w * c * batch, sizeof(float)));
    layer.norms = static_cast<float*>(calloc(h * w * c * batch, sizeof(float)));
    layer.inputs = w*h*c;
...
void resize_normalization_layer(layer *layer, int w, int h)
{
    ...
    layer->inputs = w*h*c;
    layer->outputs = layer->inputs;
    layer->output = static_cast<float*>(realloc(layer->output, h * w * c * batch * sizeof(float)));
    layer->delta = static_cast<float*>(realloc(layer->delta, h * w * c * batch * sizeof(float)));
    layer->squared = static_cast<float*>(realloc(layer->squared, h * w * c * batch * sizeof(float)));
    layer->norms = static_cast<float*>(realloc(layer->norms, h * w * c * batch * sizeof(float)));
#ifdef GPU
