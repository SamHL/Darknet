...
int bbox_comparator(const void *a, const void *b)
{
    ...
    free_image(im2);
    free(X);
    if (predictions[classC*2] > predictions[classC*2+1]){
        return 1;
    }
    return -1;
}

void bbox_update(sortable_bbox *a, sortable_bbox *b, int classC, int result)
{
    int k = 32;
    float EA = 1./(1+pow(10, (b->elos[classC] - a->elos[classC])/400.));
    float EB = 1./(1+pow(10, (a->elos[classC] - b->elos[classC])/400.));
    float SA = result ? 1 : 0;
    float SB = result ? 0 : 1;
    a->elos[classC] += k*(SA - EA);
    b->elos[classC] += k*(SB - EB);
}

void bbox_fight(network net, sortable_bbox *a, sortable_bbox *b, int classes, int classC)
{
...
void SortMaster3000(char *filename, char *weightfile)
{
    ...
    int N = plist->size;
    free_list(plist);
    sortable_bbox *boxes = static_cast<sortable_bbox*>(calloc(N, sizeof(sortable_bbox)));
...
void BattleRoyaleWithCheese(char *filename, char *weightfile)
{
    ...
    for(i = 0; i < N; ++i){
        ...
        boxes[i].elos = static_cast<float*>(calloc(classes, sizeof(float)));
        for(j = 0; j < classes; ++j){
            boxes[i].elos[j] = 1500;
        }
    }
    ...
    for(round = 1; round <= 4; ++round){
        ...
        printf("Round: %d\n", round);
        shuffle(reinterpret_cast<char*>(boxes), N, sizeof(sortable_bbox));
        for(i = 0; i < N/2; ++i){
    ...
    for (classC = 0; classC < classes; ++classC){

        N = total;
        current_class = classC;
        ...
        for(round = 1; round <= 100; ++round){
            ...
            sorta_shuffle(reinterpret_cast<char*>(boxes), N, sizeof(sortable_bbox), 10);
            for(i = 0; i < N/2; ++i){
                bbox_fight(*net, boxes+i*2, boxes+i*2+1, classes, classC);
            }
            qsort(boxes, N, sizeof(sortable_bbox), elo_comparator);
        ...
        char buff[256];
        sprintf(buff, "results/battle_%d.log", classC);
        FILE *outfp = fopen(buff, "w");
        for(i = 0; i < N; ++i){
            fprintf(outfp, "%s %f\n", boxes[i].filename, boxes[i].elos[classC]);
        }
        fclose(outfp);
    }
