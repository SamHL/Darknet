...
layer make_lstm_layer(int batch, int inputs, int outputs, int steps, int batch_normalize, int adam)
{
    ...
    *(l.wo) = make_connected_layer(batch*steps, outputs, outputs, LINEAR, batch_normalize, adam);
    ...
    l.output = static_cast<float*>(calloc(outputs*batch*steps, sizeof(float)));
    l.state = static_cast<float*>(calloc(outputs*batch, sizeof(float)));

    l.forward = forward_lstm_layer;
    l.update = update_lstm_layer;

    l.prev_state_cpu = static_cast<float*>(calloc(batch*outputs, sizeof(float)));
    l.prev_cell_cpu = static_cast<float*>(calloc(batch*outputs, sizeof(float)));
    l.cell_cpu = static_cast<float*>(calloc(batch*outputs*steps, sizeof(float)));

    l.f_cpu = static_cast<float*>(calloc(batch*outputs, sizeof(float)));
    l.i_cpu = static_cast<float*>(calloc(batch*outputs, sizeof(float)));
    l.g_cpu = static_cast<float*>(calloc(batch*outputs, sizeof(float)));
    l.o_cpu = static_cast<float*>(calloc(batch*outputs, sizeof(float)));
    l.c_cpu = static_cast<float*>(calloc(batch*outputs, sizeof(float)));
    l.h_cpu = static_cast<float*>(calloc(batch*outputs, sizeof(float)));
    l.temp_cpu = static_cast<float*>(calloc(batch*outputs, sizeof(float)));
    l.temp2_cpu = static_cast<float*>(calloc(batch*outputs, sizeof(float)));
    l.temp3_cpu = static_cast<float*>(calloc(batch*outputs, sizeof(float)));
    l.dc_cpu = static_cast<float*>(calloc(batch*outputs, sizeof(float)));
    l.dh_cpu = static_cast<float*>(calloc(batch*outputs, sizeof(float)));
