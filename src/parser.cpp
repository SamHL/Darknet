...
int *parse_yolo_mask(char *a, int *num)
{
    ...
    if(a){
        ...
        mask = static_cast<int*>(calloc(n, sizeof(int)));
        for(i = 0; i < n; ++i){
            int val = atoi(a);
            mask[i] = val;
            a = strchr(a, ',')+1;
...
route_layer parse_route(list *options, size_params params, network *net)
{
    ...
    int *layers = static_cast<int*>(calloc(n, sizeof(int)));
    int *sizes = static_cast<int*>(calloc(n, sizeof(int)));
    for(i = 0; i < n; ++i){
        int index = atoi(l);
        l = strchr(l, ',')+1;
        if(index < 0) index = params.index + index;
...
void parse_net_options(list *options, network *net)
{
    ...
    if(net->policy == STEP){
        ...
        int *steps = static_cast<int*>(calloc(n, sizeof(int)));
        float *scales = static_cast<float*>(calloc(n, sizeof(float)));
        for(i = 0; i < n; ++i){
            int step    = atoi(l);
            float scale = atof(p);
            l = strchr(l, ',')+1;
...
list *read_cfg(char *filename)
{
    ...
    while((line=fgetl(file)) != 0){
        ...
        switch(line[0]){
            case '[':
                current = static_cast<section*>(malloc(sizeof(section)));
...
{
    float *transpose = static_cast<float*>(calloc(rows*cols, sizeof(float)));
    int x, y;
    for(x = 0; x < rows; ++x){
        for(y = 0; y < cols; ++y){
