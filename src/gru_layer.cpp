...
layer make_gru_layer(int batch, int inputs, int outputs, int steps, int batch_normalize, int adam)
{
    ...
    *(l.wr) = make_connected_layer(batch*steps, outputs, outputs, LINEAR, batch_normalize, adam);
    ...
    l.uh = static_cast<layer*>(malloc(sizeof(layer)));
    fprintf(stderr, "\t\t");
    *(l.uh) = make_connected_layer(batch*steps, inputs, outputs, LINEAR, batch_normalize, adam);
    l.uh->batch = batch;

    l.wh = static_cast<layer*>(malloc(sizeof(layer)));
    fprintf(stderr, "\t\t");
    *(l.wh) = make_connected_layer(batch*steps, outputs, outputs, LINEAR, batch_normalize, adam);
    ...
    l.delta = static_cast<float*>(calloc(outputs*batch*steps, sizeof(float)));
    l.state = static_cast<float*>(calloc(outputs*batch, sizeof(float)));
    l.prev_state = static_cast<float*>(calloc(outputs*batch, sizeof(float)));
    l.forgot_state = static_cast<float*>(calloc(outputs*batch, sizeof(float)));
    l.forgot_delta = static_cast<float*>(calloc(outputs*batch, sizeof(float)));

    l.r_cpu = static_cast<float*>(calloc(outputs*batch, sizeof(float)));
    l.z_cpu = static_cast<float*>(calloc(outputs*batch, sizeof(float)));
    l.h_cpu = static_cast<float*>(calloc(outputs*batch, sizeof(float)));
