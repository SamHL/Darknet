...
{
    char **random_paths = static_cast<char**>(calloc(n, sizeof(char*)));
    int i;
    pthread_mutex_lock(&mutex);
    for(i = 0; i < n; ++i){
        int index = rand()%m;
...
{
    char **replace_paths = static_cast<char**>(calloc(n, sizeof(char*)));
    int i;
    for(i = 0; i < n; ++i){
        char replaced[4096];
...
matrix load_image_paths_gray(char **paths, int n, int w, int h)
{
    ...
    matrix X;
    X.rows = n;
    X.vals = static_cast<float**>(calloc(X.rows, sizeof(float*)));
    X.cols = 0;
...
box_label *read_boxes(char *filename, int *n)
{
    ...
    int count = 0;
    int size = 64;
    box_label *boxes = static_cast<box_label*>(calloc(size, sizeof(box_label)));
    while(fscanf(file, "%d %f %f %f %f", &id, &x, &y, &w, &h) == 5){
        if(count == size) {
            size = size * 2;
            boxes = static_cast<box_label*>(realloc(boxes, size*sizeof(box_label)));
        }
        boxes[count].id = id;
        boxes[count].x = x;
        boxes[count].y = y;
...
data load_data_seg(int n, char **paths, int m, int w, int h, int classes, int min, int max, float angle, float aspect, float hue, float saturation, float exposure, int div)
{
    ...
    d.X.rows = n;
    d.X.vals = static_cast<float**>(calloc(d.X.rows, sizeof(float*)));
    d.X.cols = h*w*3;


    d.y.rows = n;
    d.y.cols = h*w*classes/div/div;
    d.y.vals = static_cast<float**>(calloc(d.X.rows, sizeof(float*)));
...
data load_data_compare(int n, char **paths, int m, int classes, int w, int h)
{
    ...
    for(i = 0; i < n; ++i){
        ...
        d.X.vals[i] = static_cast<float*>(calloc(d.X.cols, sizeof(float)));
...
{
    pthread_t thread;
    struct load_args *ptr = static_cast<struct load_args*>(calloc(1, sizeof(struct load_args)));
    *ptr = args;
...
void *load_threads(void *ptr)
{
    ...
    int total = args.n;
    free(ptr);
    data *buffers = static_cast<data*>(calloc(args.threads, sizeof(data)));
    pthread_t *threads = static_cast<pthread_t*>(calloc(args.threads, sizeof(pthread_t)));
...
data load_data_super(char **paths, int n, int m, int w, int h, int scale)
{
    ...
    int i;
    d.X.rows = n;
    d.X.vals = static_cast<float**>(calloc(n, sizeof(float*)));
    d.X.cols = w*h*3;

    d.y.rows = n;
    d.y.vals = static_cast<float**>(calloc(n, sizeof(float*)));
...
data select_data(data *orig, int *inds)
{
    ...
    d.X.vals = static_cast<float**>(calloc(orig[0].X.rows, sizeof(float *)));
    d.y.vals = static_cast<float**>(calloc(orig[0].y.rows, sizeof(float *)));
    int i;
...
{
    data *ds = static_cast<data*>(calloc(divs*divs, sizeof(data)));
    int i, j;
...
matrix concat_matrix(matrix m1, matrix m2)
{
    ...
    m.vals = static_cast<float**>(calloc(m1.rows + m2.rows, sizeof(float*)));
    for(i = 0; i < m1.rows; ++i){
...
data get_random_data(data d, int num)
{
    ...
    r.X.vals = static_cast<float**>(calloc(num, sizeof(float *)));
    r.y.vals = static_cast<float**>(calloc(num, sizeof(float *)));
...
{
    data *split = static_cast<data*>(calloc(2, sizeof(data)));
    int i;
    ...
    train.X.vals = static_cast<float**>(calloc(train.X.rows, sizeof(float*)));
    test.X.vals = static_cast<float**>(calloc(test.X.rows, sizeof(float*)));
    train.y.vals = static_cast<float**>(calloc(train.y.rows, sizeof(float*)));
    test.y.vals = static_cast<float**>(calloc(test.y.rows, sizeof(float*)));
