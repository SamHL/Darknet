...
local_layer make_local_layer(int batch, int h, int w, int c, int n, int size, int stride, int pad, ACTIVATION activation)
{
    ...
    l.weights = static_cast<float*>(calloc(c*n*size*size*locations, sizeof(float)));
    l.weight_updates = static_cast<float*>(calloc(c*n*size*size*locations, sizeof(float)));

    l.biases = static_cast<float*>(calloc(l.outputs, sizeof(float)));
    l.bias_updates = static_cast<float*>(calloc(l.outputs, sizeof(float)));

    // float scale = 1./sqrt(size*size*c);
    float scale = sqrt(2./(size*size*c));
    for(i = 0; i < c*n*size*size; ++i) l.weights[i] = scale*rand_uniform(-1,1);

    l.output = static_cast<float*>(calloc(l.batch*out_h * out_w * n, sizeof(float)));
    l.delta  = static_cast<float*>(calloc(l.batch*out_h * out_w * n, sizeof(float)));
